...
Alpha is intentionally not the stable package. It is where wrapper-only experiments live first, including the current configurable Sure import limits for larger NDJSON/import testing. Alpha images publish under the dedicated [Docker Hub](https://hub.docker.com/r/jsonbored/sure-aio-alpha) and [GHCR](https://github.com/users/JSONbored/packages/container/package/sure-aio-alpha) packages, with GitHub prereleases tagged as `sure-alpha/<version>`. See [docs/alpha-lane.md](docs/alpha-lane.md) before pointing it at important data.
...
- Alpha publishes to the dedicated `jsonbored/sure-aio-alpha` image repo with only `latest-alpha` and an explicit AIO revision tag such as `0.7.1-alpha.7-aio.5`.
