...
- Alpha publishes to the separate `jsonbored/sure-aio-alpha` image with `latest-alpha`, the upstream alpha version, an explicit AIO revision tag such as `0.7.1-alpha.7-aio.1`, and `sha-<commit>`.
