...
- Alpha publishes to the shared `jsonbored/sure-aio` image repo with `latest-alpha`, the upstream alpha version, an explicit AIO revision tag such as `0.7.1-alpha.7-aio.4`, and `sha-alpha-<commit>`.
